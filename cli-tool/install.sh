...
if ! command -v ollama &> /dev/null; then
    ...
    if [ "$PROVIDER" = "ollama" ]; then
        ...
        echo -e "${YELLOW}  Skipped. Z.ai (cloud, no key) works without Ollama.${NC}"
        echo -e "${YELLOW}  Install later: curl -fsSL https://ollama.com/install.sh | sh${NC}"
    fi
else
...
echo ""
echo -e "  ${YELLOW}1. Z.ai (zero-setup, no API key, no download):${NC}"
echo -e "     Default — just run 'free-cli' and start chatting."
echo -e "     Uses GLM-4.6 (flagship) or GLM-4-Flash."
echo ""
echo -e "  ${YELLOW}2. Ollama (100% local, no internet):${NC}"
echo -e "     ${CYAN}ollama serve${NC}  (start daemon)"
echo -e "     ${CYAN}ollama pull glm4:9b${NC}  (download model)"
echo -e "     ${CYAN}free-cli${NC}  (then type: /provider ollama)"
echo ""
echo -e "  ${YELLOW}3. Cloud API providers (free tier with key):${NC}"
echo -e "     ${CYAN}free-cli${NC}"
echo -e "     Inside: /apikey openrouter <key>   (get: https://openrouter.ai/keys)"
echo -e "             /provider openrouter"
echo ""
echo -e "  ${CYAN}Get free API keys:${NC}"
echo -e "    OpenRouter:  https://openrouter.ai/keys"
echo -e "    Google:      https://aistudio.google.com/app/apikey"
echo -e "    Groq:        https://console.groq.com/keys"
echo -e "    HuggingFace: https://huggingface.co/settings/tokens"
echo ""
